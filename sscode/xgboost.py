...
def XGBoost_Regression_dmlc(
    ...
    print('\n XGBoost_DMLC regression with {} parameters... \n'.format(
        xgboost_parameters
    )) if verbose else None
    # get data in xgboost format (DMatrix)
    X_train_DM = xgb.DMatrix(X_train,label=y_train)
    X_test_DM = xgb.DMatrix(X_test,label=y_test)
    # specify parameters via map
    num_round = xgboost_parameters.pop('num_round')
    bst = xgb.train(xgboost_parameters, X_train_DM, num_round)
    # make prediction
    prediction = bst.predict(X_test_DM)
